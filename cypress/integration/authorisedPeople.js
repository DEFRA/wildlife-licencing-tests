...
describe("Add authorised People", () => {
  it(" 1 Verify happy path - Add an authorised People", () => {
    ...
    cy.fixture("users.json").then((users) => {
      ...
      cy.get("#yes-no-2").click();
      cy.get("#continue").click();
      cy.get("#main-content > div > div > form > fieldset > span > ol > li:nth-child(2) > ul > li:nth-child(5)").contains(
        "COMPLETED");
        cy.get("#main-content > div > div > form > fieldset > span > ol > li:nth-child(2) > ul > li:nth-child(5)").contains(
...
it(" 4 Verify happy path - Add multiple authorised People and change name", () => {
  ...
  cy.fixture("users.json").then((users) => {
    ...
    cy.get("#continue").click();
    cy.get("#main-content > div > div > form > fieldset > dl:nth-child(9) > div:nth-child(1) > dd.govuk-summary-list__actions > a").click();
    cy.get("#name").type(faker.name.firstName() + faker.name.lastName());
    cy.get("#continue").click();
    cy.get("#yes-no-2").click();
    cy.get("#continue").click();
