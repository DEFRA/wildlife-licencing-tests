...
describe("Page title verification", () => {
  ...
  it("3 and verify application season period screen", () => {
    cy.request("https://new-tst.aws.defra.cloud/set-sysdate?iso-string=2023-11-30T23:59:59.000Z");
    cy.request("https://new-tst.aws.defra.cloud/reset?username=user1@email.com");
    cy.visit(Cypress.env("baseUrl") + Cypress.env("login"));
    cy.fixture("users.json").then((users) => {
      cy.get("#username").type(users.email3);
      ...
      cy.get("#yes-no-2").click();
      cy.get("#continue").click();
      // cy.get("h1.govuk-fieldset__heading").contains("Licences are typically issued between July and November to protect badger welfare");
  ...
  it("4 and verify application season period screen", () => {
    cy.request("https://new-tst.aws.defra.cloud/set-sysdate?iso-string=2023-03-31T23:59:59.000Z");
    cy.request("https://new-tst.aws.defra.cloud/reset?username=user1@email.com");
    cy.visit(Cypress.env("baseUrl") + Cypress.env("login"));
    cy.fixture("users.json").then((users) => {
      cy.get("#username").type(users.email3);
