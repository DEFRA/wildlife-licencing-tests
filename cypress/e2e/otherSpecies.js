  ...
  cy.fixture("users.json").then((users) => {
    ...
    cy.get("#continue").click();
      cy.fixture("testdata.json").then((testdata) => {
        ...
        ).contains(testdata.otherspecies);
        cy.get(
            "fieldset.govuk-fieldset"
          ).contains(testdata.otherspeciestext1);
          cy.get(
            "fieldset.govuk-fieldset"
          ).contains(testdata.otherspeciestext2);
      });
    });
  });
    ...
    cy.fixture("users.json").then((users) => {
      ...
      cy.get("#continue").click();
        cy.fixture("testdata.json").then((testdata) => {
          cy.get(
            "fieldset.govuk-fieldset"
          ).contains(testdata.checkdevelopmentlinktext);
          cy.get(
              "fieldset.govuk-fieldset"
            ).contains(testdata.checkdevelopment);
